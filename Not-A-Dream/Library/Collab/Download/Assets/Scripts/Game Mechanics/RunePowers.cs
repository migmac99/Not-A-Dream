...
public class RunePowers : MonoBehaviour {
	...
	public bool Rune_5_Particles_Next;
	...
	[Header ("╚═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════")]
	[Space (20)]
	[Header ("╔═════════════════[Other Map Stuff]═════════════════════════════════════════════════════════════════════════════════════════════")]
	[Space (10)]

	public GameObject CurrentMapCheckpoint;
	...
	void RuneIsCollected () {
		if (Rune_1.GetComponent<RuneFollowing> ().isCollected) { //If the rune has been collected (Reference to the RuneFollowing script inside this rune)
			...
			GameManager.Instance.UnlockedRune[1] = true; //Sets the GameManager Instance know the status of rune collection
		...
		if (Rune_2.GetComponent<RuneFollowing> ().isCollected) {
			...
			GameManager.Instance.UnlockedRune[2] = true;
			Power_2 ();
		} else {
		...
		if (Rune_3.GetComponent<RuneFollowing> ().isCollected) {
			...
			GameManager.Instance.UnlockedRune[3] = true;
		...
		if (Rune_4.GetComponent<RuneFollowing> ().isCollected) {
			...
			GameManager.Instance.UnlockedRune[4] = true;
			Power_4 ();
		} else {
		...
		if (Rune_5.GetComponent<RuneFollowing> ().isCollected) {
			...
			GameManager.Instance.UnlockedRune[5] = true;
			Power_5 ();
		} else {
	...
	void Power_5 () {
		...
		if (Rune_5_State == "Enabling") {
			...
			Rune_5_State = "Charging";
		}

		if (Rune_5_State == "Charging") {

			//StartCoroutine (Countdown (1f, () => { Rune_5_Particles_Current += 1; }));
			//for (int i = 0; i <= Rune_5_Particles.Length; i++) {
			//Rune_5_Particles_Next = false;
			//StartCoroutine (Countdown (2f, () => { Rune_5_Particles_Next = true; }));
			//for (int CurrentRuneParticle = i; CurrentRuneParticle <= Rune_5_Particles.Length; CurrentRuneParticle++) {
			//Rune_5_Particles[CurrentRuneParticle].GetComponent<ParticleFollowing> ().CurrentWaypointID += 1;

			// 	Debug.Log ("CurrentRuneParticle " + CurrentRuneParticle + "         Current i " + i);
			// }
			//while (!Rune_5_Particles_Next) { }
			//}
			Rune_5_State = "Charged";
		}
